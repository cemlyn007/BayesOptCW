...
class AcquisitionFunction(metaclass=abc.ABCMeta):
    def compute_arg_max(self,
        ...
        def evaluate_gaussian_process(data_point: np.ndarray) -> float:
            return (-1 * self.evaluate(gaussian_process, objective_function,
                                       data_point)).item()

        boundaries = tuple(map(itemgetter(0),
                               objective_function.dataset_bounds))

        if len(boundaries) == 1:
            data_points, = objective_function.get_mesh_grid([100])
            data_points = data_points.reshape((-1, 1))
            evaluations = self.evaluate(gaussian_process, objective_function,
                                        data_points)
            ...
            result = optimize.fmin_l_bfgs_b(func=evaluate_gaussian_process,
                                            x0=initial_point,
                                            bounds=boundaries,
                                            approx_grad=True)
            min_point = result[0]
